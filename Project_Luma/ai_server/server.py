...
app = FastAPI(
    ...
    description="Distributed Generative AI Node with LoRA Auto-Trigger Injection, VRAM Safeguards, and Task Cancellation",
    version="1.1.0",
    lifespan=lifespan
)
...
class GenerateRequest(BaseModel):
    ...
    steps: Optional[int] = AIConfig.DEFAULT_STEPS
    cfg_scale: Optional[float] = AIConfig.DEFAULT_CFG
    width: Optional[int] = AIConfig.DEFAULT_WIDTH
    height: Optional[int] = AIConfig.DEFAULT_HEIGHT
...
def handle_txt2img_inference(data: dict) -> str:
    ...
    draw.text((36, 115), f"Model: {data.get('model')} | LoRA: {lora_id or 'None'}", fill=(130, 160, 200))
